...
namespace NEAT
{
	...
	Genome::Genome(int id, std::ifstream &iFile)
	{
		...
		while (!done && std::getline(iFile, curline))
		{
			...
			else if (curword == "trait")
			{
				...
				newtrait = new Trait(argline.c_str());
			...
			else if (curword == "node")
			{
				...
				newnode = new NNode(argline.c_str(), traits);
			...
			else if (curword == "gene")
			{
				...
				newgene = new Gene(argline.c_str(), traits, nodes);
	...
	bool Genome::mutate_add_node(std::vector<Innovation *> &innovs, int &curnode_id, double &curinnov)
	{
		...
		if (false)
		{
			...
			while (((thegene != genes.end()) && (!((*thegene)->enable))) ||
				   ((thegene != genes.end()) && (((*thegene)->lnk->in_node)->gen_node_label == BIAS)))
				++thegene;
