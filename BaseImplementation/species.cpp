...
namespace NEAT
{
	...
	bool Species::print_to_file(std::ostream &outFile) {
		...
		//outFile<<endl<<"/* Species #"<<id<<" : (Size "<<organisms.size()<<") (AF "<<ave_fitness<<") (Age "<<age<<")  */
	//"<<endl<<endl;
	// char tempbuf[1024];
	...
	bool Species::print_to_file(std::ofstream &outFile)
	{
		...
		// Print a comment on the Species info
		outFile << std::endl
				<< "/* Species #" << id << " : (Size " << organisms.size() << ") (AF " << ave_fitness << ") (Age " << age << ")  */" << std::endl
				<< std::endl;
		...
		for (curorg = organisms.begin(); curorg != organisms.end(); ++curorg)
		{
			...
			// Put the fitness for each organism in a comment
			outFile << std::endl
					<< "/* Organism #" << ((*curorg)->gnome)->genome_id << " Fitness: " << (*curorg)->fitness << " Error: " << (*curorg)->error << " */" << std::endl;
	...
	bool Species::print_to_file(std::ostream &outFile)
	{
		...
		char tempbuf[1024];
		sprintf(tempbuf, "/* Species #%d : (Size %zu) (AF %f) (Age %d)  */\n\n", id, organisms.size(), average_est, age);
